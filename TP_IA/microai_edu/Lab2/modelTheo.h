...
static inline void conv1d(const number_t input[INPUT_CHANNELS][INPUT_SAMPLES]/* IN*/,
...
static inline void dense(const number_t input[INPUT_SAMPLES]/*IN*/,const number_t kernel[FC_UNITS][INPUT_SAMPLES],/*IN*/
                         const number_t bias[FC_UNITS]/*layer IN*/,number_t output[FC_UNITS]/*OUT*/) {
    long_number_t tmp;
    for(int units_index=0;units_index<FC_UNITS;units_index++){
        //        bias+ input*kernel
        tmp =0;
        for(int input_index=0;input_index<INPUT_SAMPLES;input_index++){
            tmp+=kernel[units_index][input_index]*input[input_index]
        }
        tmp = scale_number_t(tmp);
        tmp+=bias[FC_UNITS]
        output[units_index] = clamp_to_number_t(tmp);
    }
...
void cnn(const number_t input[MODEL_INPUT_CHANNELS][MODEL_INPUT_SAMPLES],dense_output_type dense_output) {
